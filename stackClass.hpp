...
/*
    Remarks: Starts the interactive console
*/
void StackClass::start() {
    while (true) {
        // get user selection
...
*/
bool StackClass::validateSelection(const std::string& s) {
...
*/
bool StackClass::validateEntry(std::string& s) {
    // validate all characters are letters or spaces
    for (const char c : s) {
...
*/
char *StackClass::convertToCstring(const std::string& s) {
    char *cString;
...
*/
void StackClass::push(char *s, std::vector<char*>& stack, int& index) {
    // catch overflow case
    if (index == 4) {
